...
pdf:
	...
	# No figures are used; skip path rewrites
	# Build Token PDF
...
md:
	# Generate combined Markdown for Chain whitepaper
	npm install --no-fund --no-audit
	npm run fetch
	npm run generate
	rm -rf tmp/md && mkdir -p tmp/md/chain dist
	cp $(CHAIN_FILES) tmp/md/chain/
	pandoc tmp/md/chain/*.mdx \
	  --from=markdown+tex_math_dollars+tex_math_single_backslash \
	  --to=gfm+tex_math_dollars \
	  --resource-path=.:static \
	  --metadata title="MVP CHAIN Whitepaper" \
	  --metadata date="`date +%Y-%m-%d`" \
	  -o $(MD_CHAIN)
