...
function loadSnapshot() {
  if (!fs.existsSync(SNAPSHOT)) throw new Error('Missing analysis/snapshot.json. Run: npm run fetch');
...
function replaceAll(content, map) {
  return content.replace(/\{\{([A-Z0-9_]+)\}\}/g, (_m, key) => (key in map ? String(map[key]) : `{{${key}}}`));
}
